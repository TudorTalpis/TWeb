    ...
    >
      <div className="relative h-40 overflow-hidden">
        ...
        <div className="absolute top-2 left-2 flex gap-1">
          {provider.sponsored && (
            <Badge className="border-0 bg-accent text-accent-foreground text-[9px] px-2 py-0 h-4.5 rounded-full font-semibold">
              Sponsored
            </Badge>
          )}
          {provider.featured && (
            <Badge className="bg-card/80 backdrop-blur-sm text-primary border border-primary/30 text-[9px] px-2 py-0 h-4.5 rounded-full font-semibold">
              Featured
            </Badge>
          )}
        </div>
      ...
      <div className="min-h-[7.8rem] p-4 pt-5">
        ...
        {categoryLabel && <p className="mt-1 text-xs text-primary/80 leading-snug">{categoryLabel}</p>}
        <p className="mt-2 text-xs text-muted-foreground leading-relaxed line-clamp-3">{provider.description}</p>
      </div>
    </Link>
  );
}
