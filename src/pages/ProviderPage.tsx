...
const ProviderPage = () => {
  ...
  const currentUser = state.session.userId ? state.users.find((u) => u.id === state.session.userId) : null;
  const actualRole = currentUser?.role ?? state.session.role;

  const provider =
    state.providerProfiles.find((p) => p.slug === providerSlug) ||
      ...
      <div className="relative">
        ...
        <div className="absolute bottom-0 left-0 right-0">
          <div className="mx-auto max-w-6xl px-4 pb-6">
            <Link
              to="/"
              className="mb-5 inline-flex items-center gap-1.5 rounded-full border border-border/40 bg-card/80 px-3 py-1.5 text-xs text-foreground shadow-card backdrop-blur-sm transition-colors hover:bg-card"
            >
            ...
            <div className="flex flex-col sm:flex-row items-start gap-5">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2.5 flex-wrap">
                  ...
                  {provider.sponsored && (
                    <Badge className="bg-accent text-accent-foreground border-0 rounded-full px-2.5 text-xs">
                      Sponsored
                    </Badge>
                  )}
                  {provider.featured && (
                    <Badge className="bg-primary/10 text-primary border border-primary/30 rounded-full px-2.5 text-xs">
                      Featured
                    </Badge>
                  )}
                </div>
                <div className="mt-3 max-w-3xl rounded-2xl border border-border/30 bg-card/82 p-3 shadow-card backdrop-blur-sm sm:p-4">
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1.5">
                      <MapPin className="h-4 w-4 text-primary" /> {provider.location}
                    </span>
                    <span className="flex items-center gap-1.5">
                    ...
                    {categoryNames.map((name) => (
                      <Badge
                        key={name}
                        className="rounded-full border-border/50 bg-secondary text-muted-foreground px-2.5 text-xs"
                      >
                        {name}
                      </Badge>
                    ))}
          ...
          <div className="flex gap-1" role="tablist" aria-label="Provider sections">
            ...
            {(provider.galleryPhotos?.length ?? 0) > 0 && (
              <TabButton
                ...
                label={`Gallery (${provider.galleryPhotos.length})`}
              />
            )}
            <TabButton
              ...
              label={`About${reviews.length > 0 ? ` (${reviews.length})` : ""}`}
            />
          </div>
        </div>
      </div>
          ...
          <div className="grid gap-3 grid-cols-1 md:grid-cols-2">
            {services.map((svc) => {
              ...
              return (
                <div
                  key={svc.id}
                  className="flex items-center justify-between rounded-2xl border border-border/60 bg-card p-5 shadow-card card-hover group"
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2.5 flex items-center gap-3 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1 font-semibold text-foreground">
                        ...
                        {convertAndFormat(svc.price, currency)}
                      </span>
                      <span className="flex items-center gap-1">
                        <Clock className="h-3 w-3 text-muted-foreground" />
                        {svc.duration} min
                        {effectiveBuffer > 0 ? ` + ${effectiveBuffer} min buffer` : ""}
                      </span>
                    </div>
                  </div>
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1 mt-1">
                            {[...Array(5)].map((_, i) => (
                              <Star
                                key={i}
                                className={`h-3.5 w-3.5 ${i < review.rating ? "fill-warning text-warning" : "text-muted-foreground/20"}`}
                              />
                            ))}
                          </div>
                        </div>
