...
const BecomeProvider = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Save to local state
    dispatch({ type: "ADD_APPLICATION", payload: applicationPayload });

    // Save to backend DB
    try {
      await apiClient.post("/applications", {
        userId: currentUser.id, name, slug: slug.trim(),
        description, categoryIds, phone, location,
        avatar: avatar.trim(), galleryPhotos,
      });
    } catch { /* local state is the fallback */ }

    // Notify admins
    state.users.filter((u) => u.role === "ADMIN").forEach((admin) => {
      const notifPayload = {
        id: generateId(), userId: admin.id, type: "application_submitted" as const,
      ...
      try {
        apiClient.post("/notifications", {
          userId: admin.id, type: "application_submitted",
          title: notifPayload.title, message: notifPayload.message,
          linkTo: "/admin/applications",
        });
      } catch { /* ignore */ }
    });
