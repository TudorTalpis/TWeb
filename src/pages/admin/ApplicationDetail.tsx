...
const ApplicationDetail = () => {
  ...
  const linkedProvider =
    app?.status === "APPROVED"
      ? state.providerProfiles.find((p) => p.userId === app.userId && p.slug === app.slug)
      : null;
  ...
  const handleApprove = () => {
    ...
    // Update user role
    const updatedUsers = state.users.map((u) => (u.id === app.userId ? { ...u, role: "PROVIDER" as const } : u));

    // Also update the session if this user is currently logged in
    const updatedSession =
      state.session.userId === app.userId ? { userId: app.userId, role: "PROVIDER" as const } : state.session;
              ...
              <div className="flex gap-3">
                <Link to={`/admin/providers/${linkedProvider.id}`}>
                  <Button
                    variant="outline"
                    size="sm"
                    className="gap-1.5 rounded-full text-xs border-success/30 text-success hover:bg-success/5 hover:text-success hover:border-success/50"
                  >
                ...
                <Link to={`/providers/${app.slug}`}>
                  <Button
                    size="sm"
                    className="gap-1.5 rounded-full text-xs bg-success text-success-foreground hover:bg-success/90"
                  >
                    <ExternalLink className="h-3.5 w-3.5" />
                    View Public Page
                  </Button>
