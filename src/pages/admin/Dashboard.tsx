import { ArrowDownRight, ArrowUpRight, Briefcase, DollarSign, Minus, UserCheck, Users } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const statusData = statusSummary
    ...
    .map((item) => ({ name: item.label, value: item.count, color: item.color }));
                        ...
                        return (
                          <div className={TOOLTIP_CLASS}>
                            ...
                            {payload.map((entry) => {
                              ...
                              const formatted =
                                entry.dataKey === "revenue" ? `$${value.toLocaleString()}` : value.toLocaleString();
                              return (
                        ...
                        content={({ active, payload }) => {
                          ...
                          return (
                            <div className={TOOLTIP_CLASS}>
                              <p className="font-medium">{String(payload[0].name)}</p>
                              <p className="text-muted-foreground">
                                {Number(payload[0].value).toLocaleString()} providers
                              </p>
                            </div>
                          );
                        }}
          ...
          <div className={cn(PANEL_CLASS, "xl:col-span-7")}>
            ...
            {topProviders.length > 0 ? (
              ...
              <p className="py-16 text-center text-sm text-muted-foreground">
                No provider performance data available yet.
              </p>
            )}
          </div>
        </section>
